theory cache imports Main Finite_Set
begin
    ...
    next
       ...
       proof(cut_tac d1,simp,case_tac "(varOfExp ?e) \<inter>  set (map fst (statement2Assigns S) )={}")
        ...
        show " expEval (valOf (statement2Assigns S) v) s = transAux (statement2Assigns S) s v"
          by (metis b1 trans_def expEval_formEval.simps(1) substExpByStatement_def substExpVar)
        next
...
next
  ...
  apply(cut_tac b1,simp)
   ...
   with a1 a2 c1 c2 
   show "?LHS1 ?f S=?RHS1 ?f S"
   by (metis substExpByStatement_def substExp_substForm.simps(4) substFormByStatement_def)
   qed
next
  fix f1 f2
  ...
  with a1 a2 c1 c2 
   show "?LHS1 ?f S=?RHS1 ?f S"
   by (metis substExp_substForm.simps(5) substFormByStatement_def)
   qed
next
  fix f1
  ...
  by(cut_tac b1,simp)
   ...
   with a1 c1
   show "?LHS1 ?f S=?RHS1 ?f S"
   by (metis substExp_substForm.simps(6) substFormByStatement_def)
   qed
...
next   
  ...
  with a1 a2 c1 c2 
   show "?LHS1 ?f S=?RHS1 ?f S"
   by (metis substExp_substForm.simps(8) substFormByStatement_def)
...
next
  ...
  with a1 a2 c1 c2 
   show "?LHS1 ?f S=?RHS1 ?f S"
   by (metis substExp_substForm.simps(7) substFormByStatement_def)
...
by (metis assms valOfLemma2Aux varsOfSent1) 
 ...
 lemma noEffectValOfForallStatementAux[intro]:
  shows "( \<forall>i. (i\<le>N \<longrightarrow>v \<notin>  (varOfSent (ps i)) ))\<longrightarrow> valOf (statement2Assigns  (forallSent (down N) ps)) v=IVar v" (is "?P N")
  proof(induct_tac N)
    show "?P 0"
      apply simp
      done
   next
    ...
    show "?P (Suc N)"
     apply simp
      apply(cut_tac b1,rule impI)
      by (metis cache.moreSent downNIsNotEmpty le_Suc_eq valOfLemma4 varsOfSent1)
     
      
 qed     
 lemma noEffectValOfForallStatement[simp,intro]:
  shows "( \<forall>i. (i\<le>N \<longrightarrow>v \<notin>  (varOfSent (ps i))) ) \<Longrightarrow>  valOf (statement2Assigns  (forallSent (down N) ps)) v=IVar v" 
  by(metis noEffectValOfForallStatementAux)
  
 lemma noEffectValOfForallStatement1[simp,intro]:
  shows "( \<forall>i. (i\<le>N \<longrightarrow>v \<notin>  (varOfSent (ps i))) ) \<Longrightarrow>  expEval ( valOf (statement2Assigns  (forallSent (down N) ps)) v) s=s  v"
by (metis expEval_formEval.simps(1) noEffectValOfForallStatement) 
...
lemma valOf8Aux:
 "i \<le>N \<longrightarrow> (valOf (statement2Assigns  (forallSent (down N)  (\<lambda>i. assign (Para v i, e i)))) (Para v i)) =   (  e i) " (is "?P N")
proof(induct_tac N)
  show "?P 0" 
  by(simp)
  next
    ...
    assume b1:"?P N"
    show "?P (Suc N)"
    by (metis ForallSentForm valOfLemma)
 qed     
 
 lemma valOf81[simp,intro]: "i \<le>N \<Longrightarrow> (valOf (statement2Assigns  (forallSent (down N)  (\<lambda>i. assign (Para v i, e i)))) (Para v i)) =  (  e i) "
 by (metis ForallSentForm valOfLemma)
 
 lemma valOfLemma10Aux [simp,intro]:"v \<in> set (map fst   (statement2Assigns S)) \<longrightarrow>( valOf (statement2Assigns  (cat S S')) v) =    ( valOf (statement2Assigns S)) v"
  proof(induct_tac S,auto)qed
  
 lemma valOfLemma10 [simp,intro]:"v \<in> set (map fst   (statement2Assigns S)) \<Longrightarrow>( valOf (statement2Assigns  (cat S S')) v) =    ( valOf (statement2Assigns S)) v"
by (metis valOfLemma10Aux)
 
lemma paraVarsAux:"  i \<le>N\<longrightarrow>Para v i \<in> set (map fst (statement2Assigns (forallSent (down N) (\<lambda>i. assign (Para v i, e i))))) " (is "?P N")
proof(induct_tac N)
  show "?P 0" by auto
next
  fix N
  ...
  show "?P (Suc N)"
  proof
    assume b1:"i \<le> Suc N"
    show "Para v i \<in> set (map fst (statement2Assigns (forallSent (down (Suc N)) (\<lambda>i. assign (Para v i, e i)))))"
    proof -
      have b2:"\<exists> j xs. down N= j#xs"
        by (metis downNIsNotEmpty) 
        then obtain j xs where b2:"down N=j#xs" by blast
...
lemma paraVars[simp,intro]:"  i \<le>N\<Longrightarrow> Para v i \<in> set (map fst (statement2Assigns (forallSent (down N) (\<lambda>i. assign (Para v i, e i))))) "
by (metis paraVarsAux) 
  
 lemma valOf9[simp,intro]: 
 assumes a1:"i \<le>N"
 shows " expEval (valOf (statement2Assigns (cat (forallSent (down N)  (\<lambda>i. assign (Para v i, e i))) S)) (Para v i) ) s=  expEval (  e i) s" 
 proof -
   have " valOf (statement2Assigns  (cat (forallSent (down N)  (\<lambda>i. assign (Para v i, e i))) S))  (Para v i) = 
   ( valOf (statement2Assigns (forallSent (down N)  (\<lambda>i. assign (Para v i, e i)))))  (Para v i)" 
   proof(rule valOfLemma10, rule paraVars,cut_tac a1,simp)qed
   then  show "expEval (valOf (statement2Assigns (cat (forallSent (down N)  (\<lambda>i. assign (Para v i, e i))) S)) (Para v i) ) s=  expEval (  e i) s"
   by (metis ForallSentForm assms valOfLemma) 
 qed  
  end
